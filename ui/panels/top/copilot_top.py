...
class CopilotTop(QFrame):
    relay_clicked = pyqtSignal(str)
    """
    Collapsible top panel for the Copilot column.
    ...
    def __init__(self, parent=None):
        ...
        self.relay_btn.clicked.connect(lambda: self.relay_clicked.emit("copilot"))
        GlowManager().register_widget(self.relay_btn, intensity="subtle")

        self.collapse_btn = QPushButton("⮝")
        self.collapse_btn.setFixedSize(28, 28)
        self.collapse_btn.clicked.connect(lambda: ColumnManager().toggle("copilot"))
