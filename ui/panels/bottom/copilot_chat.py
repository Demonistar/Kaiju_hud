...
class CopilotChat(QFrame):
    ...
    def __init__(self, dispatcher: Dispatcher, parent=None):
        ...
        self.chat.setStyleSheet("QTextEdit { background-color: #0d1117; color: #A0C4FF; border: 1px solid #A0C4FF; }")
        self.chat.document().setDefaultStyleSheet(
            "body { color: #A0C4FF; }"
            "p { margin: 0 0 6px 0; padding: 0; color: #A0C4FF; }"
            "ul { margin: 0; padding-left: 16px; }"
            "ol { margin: 0; padding-left: 16px; }"
            "h1, h2, h3 { margin: 4px 0; color: #A0C4FF; }"
        )
    ...
    def _on_chunk(self, ai_name, chunk):
        ...
        if not self._streaming_buffer:
            self._safe_append('<span style="color:#A0C4FF"><b>COPILOT:</b></span> ')
            cursor = self.chat.textCursor()
    ...
    def _on_response(self, ai_name, content):
        ...
        self._safe_append(f'<span style="color:#A0C4FF"><b>COPILOT:</b></span> {html}')