# ui/panels/bottom/chatgpt_chat.py
...
class ChatGPTChat(QFrame):
    ...
    def __init__(self, dispatcher: Dispatcher, parent=None):
        ...
        self.chat.setStyleSheet("QTextEdit { background-color: #0d1117; color: #A0FFB0; border: 1px solid #A0FFB0; }")
        self.chat.document().setDefaultStyleSheet(
            "body { color: #A0FFB0; }"
            "p { margin: 0 0 6px 0; padding: 0; color: #A0FFB0; }"
            "ul { margin: 0; padding-left: 16px; }"
            "ol { margin: 0; padding-left: 16px; }"
            "h1, h2, h3 { margin: 4px 0; color: #A0FFB0; }"
        )
    ...
    def _on_chunk(self, ai_name, chunk):
        ...
        if not self._streaming_buffer:
            self._safe_append('<span style="color:#A0FFB0"><b>CHATGPT:</b></span> ')
            cursor = self.chat.textCursor()
    ...
    def _on_response(self, ai_name, content):
        ...
        self._safe_append(f'<span style="color:#A0FFB0"><b>CHATGPT:</b></span> {html}')